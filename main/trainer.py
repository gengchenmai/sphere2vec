...
def make_args_parser():
    ...
    parser.add_argument("--unsuper_dataset", type=str, default="inat_2018",
        help='''this is the dataset used for unsupervised learning training,
                e.g., inat_2018, inat_2017, birdsnap, nabirds, yfcc''') 
    parser.add_argument("--unsuper_meta_type", type=str, default="ebird_meta", 
        help='''this is the meta_type used for unsupervised learning training,
                e.g., orig_meta, ebird_meta''')  # orig_meta, ebird_meta

    parser.add_argument("--dataset", type=str, default="inat_2018",
        help='''e.g., inat_2018, inat_2017, birdsnap, nabirds, yfcc''') 
    parser.add_argument("--meta_type", type=str, default="ebird_meta", 
        help='''e.g., orig_meta, ebird_meta''')  # orig_meta, ebird_meta
    parser.add_argument("--eval_split", type=str, default="val",
        help='''val, test''')
    parser.add_argument("--load_val_op", type=str, default="F",
        help='''whether to pre-load the dataset with invalid dataset for final evaluation''')
    parser.add_argument("--cnn_model", type=str, default="inception_v3",
        help='''cnn model type''')
    parser.add_argument("--load_cnn_predictions", type=str, default="F",
        help='''whether to load CNN prediction on train/val/test dataset''')
    parser.add_argument("--load_cnn_features", type=str, default="F",
        help='''whether to load CNN feature (2048 dimention) on val/test dataset''') 
    parser.add_argument("--load_cnn_features_train", type=str, default="F",
        help='''whether to load CNN feature (2048 dimention) on training dataset''')
    parser.add_argument("--load_img", type=str, default="F",
        help='''whether to load images for train/val/test dataset''') 
    
    parser.add_argument("--inat2018_resolution", type=str, default="standard",
        help='''e.g., 
        ...
        ''') 
    parser.add_argument("--cnn_pred_type", type=str, default="full",
    ...
    parser.add_argument("--num_epochs_unsuper", type=int, default=30, 
    ...
    # spa enc (not change)
    parser.add_argument("--loc_encode", type=str, default="encode_cos_sin",
        help='''e.g., encode_cos_sin, encode_3D, encode_none''')
    parser.add_argument("--num_filts", type=int, default=256,
        help='spatial embedding dimension')
    ...
    parser.add_argument("--map_range", nargs='+', type=float, default=[-180, 180, -90, 90],
    ...
    parser.add_argument('--use_date_feats', type=str, default="F", 
        help="if False date feature is not used")
    parser.add_argument("--date_encode", type=str, default="encode_cos_sin",
        help='''e.g., encode_cos_sin, encode_none''')

    
    # loss
    parser.add_argument("--train_loss", type=str, default="full_loss",
        help='''appending '_user' models the user location and object affinity - see losses.py, 
                e.g.full_loss_user, full_loss''')
    parser.add_argument("--neg_rand_type", type=str, default="spherical", 
    ...
    parser.add_argument("--train_sample_method", type=str, default="stratified-fix",
    ...
    # unsupervise loss
    parser.add_argument("--unsuper_loss", type=str, default="none",
        help='''unsupervised training loss, e.g.,
            ...
            ''') 
    parser.add_argument("--num_neg_rand_loc", type=int, default=1,
    ...
    parser.add_argument("--simcse_weight", type=float, default=0.0,
        help='The weight of rand sample loss')
    parser.add_argument("--unsuper_lr", type=float, default=0.001,
        help='learning rate for unsupervised learning training')
    parser.add_argument("--do_unsuper_train", type=str, default="F",
        help='whether or not to do unsupervised training')
    parser.add_argument("--load_unsuper_model", type=str, default="F",
        help='whether or not to load the pretrained unsupervised learning model if it exists')
    parser.add_argument("--unsuper_temp_inbatch", type=float, default=1,
        help='''when unsuper_loss == contsoftmax, 
                this is the temperature used for the 1st in batch location-image loss''')
    parser.add_argument("--unsuper_temp_negloc", type=float, default=1,
        help='''when unsuper_loss == contsoftmax, 
                this is the temperature used for the 2nd negative location sampling loss''')
    parser.add_argument("--unsuper_temp_simcse", type=float, default=1,
        help='''when unsuper_loss == contsoftmax, 
                this is the temperature used for the 3rd SimCSE loss''')

    
    parser.add_argument("--unsuper_eval_frequency", type=int, default=10,
    ...
    parser.add_argument("--do_super_train", type=str, default="T",
...
def update_params(params):
    ...
    for var in ['load_val_op', 'use_layn', 'skip_connection', 'spa_enc_use_postmat', 
    'balanced_train_loader', 'use_date_feats','load_cnn_predictions', 'load_cnn_features', 'load_cnn_features_train',
    'do_unsuper_train', 'load_unsuper_model', 
    'do_super_train', 'load_super_model',
    'load_img',
               'do_epoch_save']:
        if params[var] == 'T':
            params[var] = True
...
class Trainer():
    ...
    def load_dataset_(self, params):
        ...
        op = dt.load_dataset(params, eval_split = 'val', 
    ...
    def sample_rbf_anchor_pts(self, params):
        ...
        if params['spa_enc_type'] == 'rbf':
            params['rbf_anchor_pt_ids'] = list(np.random.choice(np.arange(len(self.op['train_locs'])), 
    ...
    def make_model_file(self, params):
        ...
        param_args = ut.make_model_file_param_args(params, 
                                                spa_enc_type = params['spa_enc_type'], 
                                                get_unsuper_model_path = True)

        if param_args is None:
            params['unsuper_model_file_name'] = None
        else:
            if params['meta_type'] == '':
                params['unsuper_model_file_name'] = params["model_dir"] + "model_{}_{}_{}.pth.tar".format(params['dataset'], params['spa_enc_type'], param_args) 
            else:
                params['unsuper_model_file_name'] = params["model_dir"] + "model_{}_{}_{}_{}.pth.tar".format(params['dataset'], params['meta_type'], params['spa_enc_type'], param_args)


        # get supervised  model_path
        param_args = ut.make_model_file_param_args(params, 
                                                spa_enc_type = params['spa_enc_type'], 
                                                get_unsuper_model_path = False)
        if param_args is None:
            params['model_file_name'] = None
        else:
            if params['meta_type'] == '':
                params['model_file_name'] = params["model_dir"] + "model_{}_{}_{}.pth.tar".format(params['dataset'], params['spa_enc_type'], param_args) 
            else:
                params['model_file_name'] = params["model_dir"] + "model_{}_{}_{}_{}.pth.tar".format(params['dataset'], params['meta_type'], params['spa_enc_type'], param_args)
    ...
    def process_users(self, params):
        ...
        self.un_users, self.train_users_np = np.unique(self.op['train_users'], return_inverse=True)
    ...
    def load_ocean_mask(self):
        ...
        self.mask = np.load(get_paths('mask_dir') + 'ocean_mask.npy').astype(np.int)

    def create_dataset_data_loader(self, params, data_flag, classes, locs, dates, users, cnn_features):
        '''
        Args:
            params:
        ...
        if data_flag == "train":
            # training dataset
            dataset = LocationDataLoader(loc_feats = loc_feats, 
                                                labels = labels, 
                                                users = users_tensor, 
            ...
            if params['balanced_train_loader']:
                data_loader = torch.utils.data.DataLoader(dataset, 
                                                ...
                                                sampler=ut.BalancedSampler(classes.tolist(), 
    ...
    def create_train_sample_data_loader(self, params):
        if params["train_sample_ratio"]  < 1.0 and params["train_sample_ratio"] > 0 and params['spa_enc_type'] not in self.spa_enc_baseline_list:
            # we need to sample the training dataset for supervised learning
            train_sample_idx_file = dtul.get_sample_idx_file_path(dataset = params['dataset'], 
            ...
            if sample_seed == "fix" and os.path.exists(train_sample_idx_file):
                ...
                self.train_sample_idxs = np.load(train_sample_idx_file, allow_pickle=True)
            else:
                if sample_type == "stratified":
                    # if not we generate one and save it
                    self.train_sample_idxs, _ = dtul.get_classes_sample_idxs(classes = self.op['train_classes'], 
                                                sample_ratio = params["train_sample_ratio"])
                elif sample_type == "random":
                    num_sample = self.op['train_classes'].shape[0]
                    self.train_sample_idxs = np.sort(np.random.choice(list(range( num_sample )), size= num_sample * params["train_sample_ratio"], replace=False))
                else:
                    raise Exception(f'Unknown train_sample_method: {params["train_sample_method"]}')
            ...
            self.train_sample_dataset, self.train_sample_loader, self.train_sample_labels, self.train_sample_loc_feats, self.train_sample_users, self.train_sample_feats = self.create_dataset_data_loader(
                                    params, 
                                    data_flag = "train", 
                                    ...
                                    cnn_features = self.op["train_feats"][self.train_sample_idxs] if self.op["train_feats"] is not None else None
                                    )
        else:
            self.train_sample_dataset, self.train_sample_loader, self.train_sample_labels, self.train_sample_loc_feats, self.train_sample_users, self.train_sample_feats = None, None, None, None, None, None


    def create_train_val_data_loader(self, params):
        if params['spa_enc_type'] not in self.spa_enc_baseline_list:
            self.train_dataset, self.train_loader, self.train_labels, self.train_loc_feats, self.train_users, self.train_feats = self.create_dataset_data_loader(
                                        params, 
                                        data_flag = "train", 
            ...
            self.val_dataset, self.val_loader, self.val_labels, self.val_loc_feats , self.val_users, self.val_feats= self.create_dataset_data_loader(
                                        params, 
                                        data_flag = "val", 
            ...
            self.train_dataset, self.train_loader, self.train_labels, self.train_loc_feats, self.train_users, self.train_feats = None, None, None, None, None, None
            
            self.val_dataset, self.val_loader, self.val_labels, self.val_loc_feats , self.val_users, self.val_feats = None, None, None, None, None, None
    ...
    def create_model(self):
        if self.params['spa_enc_type'] not in self.spa_enc_baseline_list:
            ...
            if unsuper_loss == "none":
                ...
                model = models.LocationImageEncoder(loc_enc = loc_enc, 
                        train_loss = self.params["train_loss"], 
                        unsuper_loss = unsuper_loss, 
                        cnn_feat_dim = self.params["cnn_feat_dim"],
                        spa_enc_type = self.params["spa_enc_type"]).to(self.params['device'])
                return model
            else:
    ...
    def plot_groundtruth(self):
        # plot ground truth
        plt.close('all')
        plot_gt_locations(self.params, self.mask, 
    ...
    def run_unsuper_train(self):
        if self.params["unsuper_loss"] != "none" and self.params["num_epochs_unsuper"] > 0:

            # adjust the learning rate
            # we readjust the lr as the initial lr during supervised training
            for param_group in self.optimizer.param_groups:
                param_group['lr'] = self.params['unsuper_lr'] * (self.params['lr_decay'] ** self.epoch)

            for epoch in range(0, self.params['num_epochs_unsuper']):
                self.logger.info('\nUnsupervised Training Epoch\t{}'.format(epoch))
                unsupervise_train(model = self.model, 
                ...
                if epoch % self.params["unsuper_save_frequency"] == 0 and epoch != 0 and self.params["do_epoch_save"]:
    ...
    def run_super_train(self):
        ...
        if self.params["unsuper_loss"] != "none":
            ...
            # we readjust the lr as the initial lr during supervised training
            for param_group in self.optimizer.param_groups:
                param_group['lr'] = self.params['lr'] * (self.params['lr_decay'] ** self.epoch)

        if self.params["train_sample_ratio"]  < 1.0 and self.params["train_sample_ratio"] > 0 and self.train_sample_loader is not None:
        ...
        for epoch in range(self.epoch, self.epoch + self.params['num_epochs']):
            ...
            if epoch % self.params["eval_frequency"] == 0 and epoch != 0:
                self.run_eval_spa_enc_only(eval_flag_str = f"LocEnc (Epoch {epoch})", load_model = False)
    ...
    def plot_time_preidction(self):
        if self.params['use_date_feats']:
            ...
            for ii, tm in enumerate(np.linspace(0,1,13)):
                grid_pred = self.gp.dense_prediction(self.model, self.op['class_of_interest'], tm)
                op_file_name = self.op_dir + 'time/' + str(self.op['class_of_interest']).zfill(4) + '_' + str(ii) + '.jpg'
                plt.imsave(op_file_name, 1-grid_pred, cmap='afmhot', vmin=0, vmax=1)
    ...
    def load_model(self, unsuper_model = False, cur_epoch = None):
        ...
        if cur_epoch is not None:
            model_file_name = model_file_name.replace(".pth.tar", f"-Epoch-{cur_epoch}.pth.tar")
        ...
        if model_file_name is not None:
            if os.path.exists(model_file_name): 
                ...
                net_params = torch.load(model_file_name, map_location=torch.device(self.params['device']))
                ...
                self.logger.info(f"Cannot load model since it not exist - {model_file_name}")
        else:
            if unsuper_model:
    ...
    def run_est_eval_batch(self):
        ...
        self.logger.info(f'''Evaluation on {self.params["eval_split"]} with invalid sample removed''')
        ...
        self.model.load_state_dict(net_params['state_dict'])
        self.model.eval()
        val_preds_final = compute_acc_batch(val_preds = self.val_preds, 
    ...
    def save_eval(self, val_preds_final, val_pred_no_prior):
        np.savez('model_preds', val_classes=self.op['val_classes'], 
    ...
    def run_eval_final(self, spa_enc_type_list = ['no_prior'], save_eval = False, hyper_params = None, 
            eval_flag_str = ""):
        '''
        ...
        if 'no_prior' in spa_enc_type_list:
            ...
            pred_no_prior = compute_acc_batch(val_preds = op['val_preds'], 
                                                val_classes = op['val_classes'], 
                                                val_split = op['val_split'], 
                                                val_feats=None, 
                                                train_classes=None,
        ...
        if 'train_freq' in spa_enc_type_list:
            ...
            compute_acc(val_preds = op['val_preds'], 
                        val_classes = op['val_classes'], 
                        val_split = op['val_split'],  
        ...
        if 'grid' in spa_enc_type_list:
            self.logger.info('\nDiscrete grid prior')
            gp = bl.GridPrior(self.op['train_locs'], self.op['train_classes'], self.params['num_classes'], self.hyper_params)
            compute_acc(val_preds = op['val_preds'], 
                        val_classes = op['val_classes'], 
                        val_split = op['val_split'],
        ...
        if ('nn_knn' in spa_enc_type_list) or ('nn_dist' in spa_enc_type_list):
            if self.hyper_params['dist_type'] == 'haversine':
                nn_tree = BallTree(np.deg2rad(self.op['train_locs'])[:,::-1], metric='haversine')
        ...
        if 'kde' in spa_enc_type_list:
            ...
            kde_params = {}
            train_classes_kde, train_locs_kde, kde_params['counts'] = bl.create_kde_grid(self.op['train_classes'], self.op['train_locs'], self.hyper_params)
        ...
        if self.params["spa_enc_type"]  not in self.spa_enc_baseline_list:
            val_preds_final = self.run_eval_spa_enc_final(op, eval_flag_str = eval_flag_str)
    ...
    def run_eval_spa_enc_final(self, op, eval_flag_str = ""):
        ...
        self.model.eval()
        val_preds_final = compute_acc(val_preds = op['val_preds'], 
                                    val_classes = op['val_classes'], 
                                    val_split = op['val_split'],
    ...
    def run_eval_spa_enc_rank_final(self, op, eval_flag_str = ""):
        ...
        self.model.eval()
        val_preds_final, val_ranks = compute_acc_and_rank(val_preds = op['val_preds'], 
                                    val_classes = op['val_classes'], 
                                    val_split = op['val_split'],
                                    val_feats = val_loc_feats, 
                                    prior_type = spa_enc_type, 
    ...
    def run_eval_spa_enc_only(self, eval_flag_str = "LocEnc ", load_model = True):
        ...
        op = self.op
        spa_enc_type = self.params["spa_enc_type"]
        spa_enc_algs = set(ut.get_spa_enc_list() + ['geo_net'] + self.spa_enc_baseline_list)
        assert spa_enc_type in spa_enc_algs
        ...
        val_preds = compute_acc_batch(val_preds = None, 
                                        val_classes = op['val_classes'], 
                                        val_split = op['val_split'], 
                                        val_feats=val_loc_feats, 
                                        train_classes=None,
